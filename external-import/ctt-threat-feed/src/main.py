...
class CTTThreatFeed:
    def __init__(self):
        ...
        self._downloader_config = {
            "baseurl": self.get_config(
                "baseurl", config, "https://api.cyberthreattech.ru/v1/"
            ),
    ...
    def run(self):
        self.helper.log_info("Starting CTT Threat Feed connector")
    ...
    def _process_feed(self, feed_type):
        ...
        if not isinstance(result, dict):
            raise ValueError(f"Unexpected response from feed downloader: {result}")
        ...
        if status == "ok":
            ...
            raise Exception(
                f"Failed to fetch feed {feed_type}: {error_message} (status: {status})"
            )
    ...
    def _create_stix_bundle(self, filepath, feed_type):
        ...
        organization = stix2.v21.Identity(
            id=Identity.generate_id("Cyber Threat Technology", "organization"),
            name="Cyber Threat Technology",
            identity_class="organization",
            description="Threat Intelligence Company https://cyberthreattech.ru",
        )
